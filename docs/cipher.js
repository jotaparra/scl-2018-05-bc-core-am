window.cipher = { 
  ...
  decode: (offset2, message2) => {
    ...
    for (var i = 0; i < message2.length; i++)  {
      ...
      if(toAsciiCode >= 65 && toAsciiCode <= 91){      //condición para mayúsculas
        resultDecode += String.fromCharCode((toAsciiCode - 65 - parseInt(offset2) + 26*2) % 26 + 65);
      } else if(toAsciiCode >= 97 && toAsciiCode <= 122){  //condición para minúsculas
        resultDecode += String.fromCharCode((toAsciiCode - 97 - parseInt(offset2) + 26*2) % 26 + 97);
      } else 
